...
具体代码的解释见视频，直接复制粘贴下面代码肯定是用不了的，有好几处需要自行修改。
...
def generate_launch_description():
    ...
    move_group_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            FindPackageShare('car_config'), '/launch/move_group.launch.py']),  # Replace with actual MoveIt package name
    )

    # Launch RViz with MoveIt 2 configuration
    rviz_config_file = FindPackageShare('car_config')\
        .find('car_config') + '/config/moveit.rviz'  # Replace with actual path
...
```cmd
ros2 launch carbot_description gazebo.launch.py
```
